$(document).ready(function(){
	...
	$("#frm_save_subscriber").submit(function(e){
		e.preventDefault();
		var full_names=$("#full_names").val();
		var sub_mail=$("#sub_mail").val();
		var loader=$("#loader");
		input[0]="save_newsletter";
		input[1]=full_names;
		input[2]=sub_mail;
		showLoader(loader);
		$.post("request_handler",{
			...
			if(response.match("200")){
				var message="Newsletters Subscription saved Successfully";
				$("#p_error").show().html(message);
				$("#full_names").val("");
				$("#sub_mail").val("");
			}else{
				alert(response);
	...
	$(".radio_answer").click(function(event){
		var question=$(this).attr("question");
		var answer=$(this).attr("answer");
		input[0]="answer_question";
		input[1]=question;
		input[2]=answer;
		input[3]=logged_in;
		answerQuestion(input);
	});
	$("a.btn_finish_quiz").click(function(){
		var quiz_id=$(this).attr("quiz");
		if(confirm("Are you sure you want to finish a quiz ?")){
			input[0]="finish_quiz";
			input[1]=quiz_id;
			input[2]=logged_in;
			finishQuiz(input);
...
function registerStudent(form_data){	
	...
	$.ajax({  
	    ...
	    { 
	    	hideLoader(img_loader);
	    	if(data.match("200")){
	    		alert("Account Created Successfully.Please Login to Your Account Now");
